...
async function applyStaffPolicies() {
  try {
    await db.query(sql);
    console.log("Successfully applied staff appointment policies to the database!");
    process.exit(0);
  } catch (err) {
    console.error("Failed to apply staff appointment policies:", err);
    process.exit(1);
  }
}
