...
export const NotificationPrompt: React.FC<NotificationPromptProps> = ({
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-blue-200/80 dark:border-blue-900/50 bg-gradient-to-br from-white via-blue-50/40 to-indigo-50/30 dark:from-slate-900 dark:via-slate-900 dark:to-blue-950/30 p-4 sm:p-6 shadow-sm">
      {onDismiss && (
        <button
          onClick={onDismiss}
          className="absolute top-3 right-3 sm:top-4 sm:right-4 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition z-10"
          aria-label="Close"
        >
          <X className="w-4 h-4" />
        </button>
      ...
      <div className="flex flex-col sm:flex-row items-start gap-3.5 sm:gap-4 pr-6 sm:pr-0">
        <div className="relative flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-white shadow-md shadow-blue-500/20">
          <Smartphone className="w-5 h-5 sm:w-6 sm:h-6" />
          <span className="absolute -top-1 -right-1 w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full bg-emerald-500 border-2 border-white dark:border-slate-900 flex items-center justify-center text-[8px] sm:text-[9px] text-white">
            ✓
          </span>
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            {isEnabled ? (
              <span className="inline-flex items-center gap-1 text-[11px] sm:text-xs font-semibold px-2 sm:px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300 border border-emerald-300/40">
                <CheckCircle2 className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> Enabled on {platform}
              </span>
            ) : (
              <span className="inline-flex items-center gap-1 text-[11px] sm:text-xs font-medium px-2 sm:px-2.5 py-0.5 rounded-full bg-amber-100 text-amber-800 dark:bg-amber-950/60 dark:text-amber-300 border border-amber-300/40">
                <AlertCircle className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> Not Enabled
              </span>
            )}
          </div>

          <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 mb-4 max-w-xl leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2.5 sm:gap-3">
            {isEnabled ? (
              <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                <Button
                  onClick={handleTestPush}
                  disabled={isTesting}
                  className="flex-1 sm:flex-initial bg-blue-600 hover:bg-blue-700 text-white gap-2 shadow-sm font-semibold h-10 text-xs sm:text-sm rounded-xl"
                >
                ...
                <Button
                  ...
                  disabled={isLoading}
                  className="flex-1 sm:flex-initial text-slate-600 dark:text-slate-300 border-slate-300 dark:border-slate-700 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/30 dark:hover:text-red-400 h-10 text-xs sm:text-sm rounded-xl"
                >
                  Disable Alerts
                </Button>
              </div>
              ...
              <Button
                ...
                className="w-full sm:w-auto bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white gap-2 shadow-md shadow-blue-600/25 font-semibold px-5 h-11 sm:h-10 text-xs sm:text-sm rounded-xl"
              >
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 pt-1 sm:pt-0">
              <ShieldCheck className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
              <span className="text-[11px] sm:text-xs">Private & secure FCM push channel</span>
            </div>
          </div>

          {platform === 'mobile' && !isEnabled && (
            <div className="mt-3 text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 bg-white/70 dark:bg-slate-800/60 p-2.5 rounded-lg border border-slate-200/60 dark:border-slate-700/60 leading-relaxed">
